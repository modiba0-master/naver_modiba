...
## 대시보드 진행 메모 (2026-04-24)
- **이번에 반영됨:** 옵션상품명(`option_product_label`) 기준 집계·요약 Top5·상품 증감표, 해피콜 **권장 액션** `multiselect` 필터, 로드 구간 내 **단기·중기·보합·상한** 예측 컬럼, `orders-raw` **최대 45영업일** 기간 파라미터로 응답 크기 절감.
- **다음에 할 일 (보류했던 1·5번 — 이후 작업 시 우선 확인):**
  1. **고객 유입경로:** DB/API에 결제위치·채널 등 필드 노출 후 고객 탭에서 집계·분석.
  5. **원가 입력:** 관리자 전용 옵션 리스트 + 원가 위젯, `product_option_cost_master`(및 배치)와 연동 방식 확정 후 구현.

## 대시보드 진행 메모 (2026-04-25, 새벽)
- **요약 탭**
  - 기준일 선택 위젯 추가(선택일 1일 기준 리포트).
  - 전주 비교일(선택일-7일) 기준으로 옵션 상승/하락 Top 표 갱신.
  - 상승/하락 표에 전주매출 컬럼 및 하단 합계 행 추가.
- **고객 탭**
  - 최근 3개월 기준 주문패턴 컬럼 추가: 최근주문일, 주문①~⑤, 이전주문일, 재주문기간.
  - 주요 소비 옵션 정보 추가: 주요옵션(Top1), 주문횟수, 누적순매출, 매출비중.
  - 조회 기준일 위젯 추가: `기준일 + 전일` 2일 윈도우 필터.
  - 주문①~⑤는 최근주문일 제외한 직전 주문부터 표기, 날짜는 `M/D` 포맷.
  - 기본 정렬: 재주문기간(일) 오름차순(동률 시 최근주문일/우선순위 보조).
  - **중요 수정:** `buyer_id` 단독 집계에서 `buyer_id + buyer_name` 복합키 집계로 변경(마스킹 ID 충돌 완화).
- **상품 탭**
  - 기준일 위젯 추가.
  - 수량/금액/수량+금액 보기 모드 추가.
  - 수량/금액 각각 일자·주간·월간/예측 섹션으로 분리.
  - 일자 헤더를 `24일 수량`, `23일 수량` 형태로 축약.
  - 수량 집계 기준을 환산수량(`converted_quantity`)으로 통일.
  - 수량 표 정렬 기준을 기준일 수량 내림차순으로 고정.
  - 수량 일자표에 가로합(`일자 합계`) + 하단 합계 추가.
  - 수량 주간/월간 표에도 하단 합계 추가.
  - **장애 대응:** 합계 행 입력 시 float 컬럼에 빈 문자열 할당으로 발생한 pandas dtype 오류를 `NaN` 처리로 수정.
- **배포/운영**
  - 위 변경은 순차 커밋 후 `main` 푸시 및 Railway 자동배포(SUCCESS) 확인까지 완료.

## 다음 호출 즉시 확인할 보류 항목 (1/5)
- **1번 고객 유입경로**
  - 우선 `orders-raw` 응답에 유입/결제위치 필드 노출 여부 점검.
  - 필요 시 API 스키마 확장 후 고객 탭에 채널별 집계/비중 표 추가.
- **5번 원가 입력 위젯**
  - 관리자 전용 접근 제어 방식 확정(계정/비밀번호 분리 또는 role flag).
  - `product_option_cost_master` 연동 저장 경로 확정(대시보드 직접 DB 쓰기 vs 백엔드 API).
  - 저장 후 마진 반영 정책 확정(즉시 재계산/배치 반영/안내문구).
